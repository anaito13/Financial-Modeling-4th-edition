...
def get_efficient_port(S,E,c):
    ...
    port = X*alphas[opt_ind]+Y*(1-alphas[opt_ind])
    return  port, E_port[opt_ind], std_port[opt_ind]

def get_efficient_port_long(S,E,c):
    Z = np.dot(np.linalg.inv(S), E)
    X = Z / np.sum(Z)
    Z = np.dot(np.linalg.inv(S), (E-c))
    Y = Z/np.sum(Z)
    ...
    while True:
        ...
        if np.all(port > 0):
            ...
            np.delete(sharp_ratio,opt_ind)


    return port, std_port[opt_ind], alphas[opt_ind]
